...
export default function Dashboard(){
  ...
  const leads = (data?.leads||[]).filter((l:any)=>status==='ALL'||l.status===status)
    .filter((l:any)=>(l.name+l.phone+l.problem+l.extra).toLowerCase().includes(q.toLowerCase()))
    ...
    <main className="min-h-screen">
      <header className="sticky top-0 z-10 backdrop-blur bg-hyp-bg/70 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <a href="/api/auth/signout" className="text-sm underline/20 hover:underline">Sign out</a>
        </div>
      </header>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-4 gap-3">
          <input className="md:col-span-2 rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-hyp-primary/60"
            placeholder="Zoek..." value={q} onChange={e=>setQ(e.target.value)} />
          <select className="rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-hyp-primary/60" value={status} onChange={e=>setStatus(e.target.value)}>
            <option value="ALL">Alle statussen</option>
            {['A','B','C','D','E','F','CLOSED'].map(s=>(<option key={s} value={s}>{s}</option>))}
          </select>
          <button onClick={()=>setSelected({id:'NEW', name:'', phone:'', problem:'', extra:'', status:'A'})}
            className="rounded-xl px-4 py-3 bg-hyp-primary/90 hover:bg-hyp-primary text-black font-semibold transition shadow-glow">Nieuwe lead</button>
        </div>
        ...
        <div className="mt-6 overflow-x-auto rounded-2xl border border-white/10">
          <table className="w-full text-sm">
            ...
            <tbody>
            {isLoading && <tr><td className="p-4" colSpan={6}>Laden…</td></tr>}
            {leads.map((l:any)=>(
              <tr key={l.id} className="border-t border-white/10 hover:bg-white/[0.03]">
                ...
                <td className="p-3"><span className="rounded-lg px-2 py-1 bg-white/10">{l.status}</span></td>
                <td className="p-3">
                  <button className="underline/20 hover:underline mr-3" onClick={()=>setSelected(l)}>Open</button>
                  {l.status!=='CLOSED'
                    ? <button className="underline/20 hover:underline" onClick={async()=>{await fetch('/api/leads/updateStatus',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:l.id,status:'CLOSED'})}); mutate()}}>Sluit</button>
                    : <button className="underline/20 hover:underline" onClick={async()=>{await fetch('/api/leads/updateStatus',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:l.id,status:'A'})}); mutate()}}>Heropen</button>}
                </td>
              </tr>
            ))}
            {(!isLoading && leads.length===0) && <tr><td className="p-4" colSpan={6}>Geen leads gevonden.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      {selected && <LeadModal lead={selected} onClose={()=>setSelected(null)} onChanged={()=>{ setSelected(null); mutate(); }} />}
    </main>
  )
}
      ...
      <motion.div initial={{y:40,opacity:0}} animate={{y:0,opacity:1}} className="w-full max-w-2xl rounded-2xl border border-white/10 bg-hyp-card p-6 relative">
        ...
        <div className="grid md:grid-cols-2 gap-3 mt-4">
          ...
          <select className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" value={form.status} onChange={e=>setForm({...form,status:e.target.value})}>
            {['A','B','C','D','E','F','CLOSED'].map((s:string)=>(<option key={s} value={s}>{s}</option>))}
          </select>
        </div>

        <div className="mt-4 flex gap-3">
          <button className="rounded-xl px-4 py-2 bg-hyp-primary/90 hover:bg-hyp-primary text-black font-semibold shadow-glow"
            onClick={async()=>{
              ...
              onChanged()
            }}>{isNew?'Aanmaken':'Opslaan'}</button>
          {!isNew && <button className="rounded-xl px-4 py-2 border border-white/15 hover:bg-white/10"
            onClick={async()=>{
              await fetch('/api/leads/updateStatus',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:lead.id,status: form.status==='CLOSED'?'A':'CLOSED'})})
              onChanged()
            }}>{form.status==='CLOSED'?'Heropen':'Sluit'}</button>}
        </div>
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button className="rounded-xl px-4 py-2 bg-white/10 hover:bg-white/15 border border-white/10"
          onClick={async()=>{ if(!note.trim())return; await fetch('/api/leads/notes/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:leadId,note})}); setNote(''); mutate(); }}
        >Plaats</button>
      </div>
    </div>
  )
